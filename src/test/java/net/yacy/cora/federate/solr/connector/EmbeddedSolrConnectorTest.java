...
public class EmbeddedSolrConnectorTest {
    ...
    static EmbeddedInstance EMBEDDED_INSTANCE;
    ...
    public static void initTesting() {
        ...
        final File storage = new File(System.getProperty("java.io.tmpdir") + "/" + UUID.randomUUID() + "/DATA/INDEX/webportal/SEGMENTS/text/solr/");
        storage.mkdirs();
        System.out.println("setup EmeddedSolrConnector using config dir: " + solr_config.getAbsolutePath());
        try {
            EmbeddedSolrConnectorTest.EMBEDDED_INSTANCE = new EmbeddedInstance(solr_config, storage,
                    CollectionSchema.CORE_NAME, new String[]{CollectionSchema.CORE_NAME, WebgraphSchema.CORE_NAME});
            solr = new EmbeddedSolrConnector(EmbeddedSolrConnectorTest.EMBEDDED_INSTANCE);
            solr.clear(); // delete all documents in index (for clean testing)
        } catch (final IOException ex) {
            Assert.fail("IOException starting Jetty");
        }
    }

    @AfterClass
    public static void finalizeTesting() {
        EmbeddedSolrConnectorTest.EMBEDDED_INSTANCE.close();
    }
